...
object MyHistogram extends App with HTTPServer {
  ...
  val port = 8080
  startServer
  val requestLatency = Histogram.build.name("requests_latency_seconds").help("Request latency in seconds.").register
...
object MyTimer extends App with HTTPServer {
  ...
  val requestLatency = Summary.build.name("requests_latency_seconds").help("Request latency in seconds.").register
